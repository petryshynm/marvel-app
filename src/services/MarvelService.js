...
const useMarvelService = () => {
    ...
    const _apiPath = 'https://gateway.marvel.com:443/v1/public/';
    // _apiKey = '43d79aef7bf47ccf622c7f460a8a2a82';
    const _apiKey = 'apikey=c5d6fc8b83116d92ed468ce36bac6c62';
    const _baseOffset = 210
    
    const getAllCharacters = async (offset = _baseOffset) => {
        const res = await request(`${_apiPath}characters?limit=9&offset=${offset}&${_apiKey}`);
        return res.data.results.map(_transfromCharacter);
    }
    const getCharacter = async (id) => {
        const res = await request(`${_apiPath}characters/${id}?${_apiKey}`)
        return _transfromCharacter(res.data.results[0]);
    }
    
    const getAllComics = async (offset = _baseOffset) => {
        const res = await request(`${_apiPath}comics?orderBy=issueNumber&limit=8&offset=${offset}&${_apiKey}`);
        return res.data.results.map(_transformComics);
    }

    const getComics = async (id) => {
        const res = await request(`${_apiPath}comics/${id}?${_apiKey}`);
